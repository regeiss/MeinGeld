...
final class FirebaseService: FirebaseServiceProtocol, @unchecked Sendable {
  ...
  private let logger = Logger(
    subsystem: "com.personalfinance.app",
  ...
  func configure() {
    ...
    guard
      ...
      logger.error(
        "GoogleService-Info.plist não encontrado. Firebase não será configurado."
      )
      return
    }
  ...
  func logEvent(_ event: AnalyticsEvent) {
    guard isConfigured else {
      logger.warning(
        "Firebase não configurado. Evento não será enviado: \(event.name)"
      )
      return
    }
  ...
  func recordError(_ error: Error, context: String) {
    guard isConfigured else {
      logger.warning(
        "Firebase não configurado. Erro não será enviado: \(error.localizedDescription)"
      )
      return
    }
    ...
    logger.error(
      "Crashlytics error recorded: \(error.localizedDescription) - Context: \(context)"
    )

    #if DEBUG
      print(
        "💥 CRASHLYTICS ERROR: \(error.localizedDescription) - Context: \(context)"
      )
    #endif
  }
  ...
  func recordNonFatalError(_ error: Error, context: String) {
    guard isConfigured else {
      logger.warning(
        "Firebase não configurado. Erro não-fatal não será enviado: \(error.localizedDescription)"
      )
      return
    }

    let nsError = error as NSError
    let userInfo = nsError.userInfo.merging(["context": context]) { _, new in
      new
    }
    ...
    logger.warning(
      "Crashlytics non-fatal error recorded: \(error.localizedDescription) - Context: \(context)"
    )

    #if DEBUG
      print(
        "⚠️ CRASHLYTICS NON-FATAL: \(error.localizedDescription) - Context: \(context)"
      )
    #endif
  }
  ...
  func setUserID(_ userID: String) {
    guard isConfigured else {
      logger.warning(
        "Firebase não configurado. UserID não será definido: \(userID)"
      )
      return
    }
  ...
  func setUserProperty(_ value: String?, forName name: String) {
    guard isConfigured else {
      logger.warning(
        "Firebase não configurado. Propriedade não será definida: \(name)"
      )
      return
    }
