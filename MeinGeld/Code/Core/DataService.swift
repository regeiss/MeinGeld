...
final class DataService: DataServiceProtocol {
    ...
    func fetchAccounts(for user: User) async throws -> [Account] {
        do {
            let descriptor = FetchDescriptor<Account>(
                predicate: #Predicate<Account> { account in account.user?.id == user.id && account.isActive },
    ...
    func fetchTransactions(for user: User, limit: Int? = nil, offset: Int? = nil) async throws -> [Transaction] {
        do {
            var descriptor = FetchDescriptor<Transaction>(
                predicate: #Predicate<Transaction> { transaction in transaction.account?.user?.id == user.id },
    ...
    func fetchTransactions(for account: Account, limit: Int? = nil, offset: Int? = nil) async throws -> [Transaction] {
        do {
            var descriptor = FetchDescriptor<Transaction>(
                predicate: #Predicate<Transaction> { transaction in transaction.account?.id == account.id },
                sortBy: [SortDescriptor(\.date, order: .reverse)]
            )
    ...
    func fetchTransactionsByCategory(_ category: TransactionCategory, for user: User) async throws -> [Transaction] {
        do {
            let descriptor = FetchDescriptor<Transaction>(
                predicate: #Predicate<Transaction> { transaction in transaction.category == category && transaction.account?.user?.id == user.id },
                sortBy: [SortDescriptor(\.date, order: .reverse)]
            )
            return try modelContext.fetch(descriptor)
        } catch {
    ...
    func fetchTransactionsByDateRange(from startDate: Date, to endDate: Date, for user: User) async throws -> [Transaction] {
        do {
            let descriptor = FetchDescriptor<Transaction>(
                predicate: #Predicate<Transaction> { transaction in transaction.date >= startDate && transaction.date <= endDate && transaction.account?.user?.id == user.id },
                sortBy: [SortDescriptor(\.date, order: .reverse)]
            )
            return try modelContext.fetch(descriptor)
        } catch {
    ...
    func fetchBudgets(for user: User) async throws -> [Budget] {
        do {
            let descriptor = FetchDescriptor<Budget>(
                predicate: #Predicate<Budget> { budget in budget.user?.id == user.id },
    ...
    func fetchBudget(for category: TransactionCategory, month: Int, year: Int, user: User) async throws -> Budget? {
        do {
            let descriptor = FetchDescriptor<Budget>(
                predicate: #Predicate<Budget> { budget in budget.category == category && budget.month == month && budget.year == year && budget.user?.id == user.id }
            )
    ...
    func fetchExpensesByCategory(for user: User, month: Int, year: Int) async throws -> [String: Decimal] {
        do {
            ...
            let transactions = try await fetchTransactionsByDateRange(from: startDate, to: endDate, for: user)
    ...
    func fetchIncomeVsExpenses(for user: User, months: Int) async throws -> [(month: String, income: Decimal, expenses: Decimal)] {
        do {
            ...
            for i in 0..<months {
                guard let monthDate = calendar.date(byAdding: .month, value: -i, to: currentDate),
                ...
                let transactions = try await fetchTransactionsByDateRange(from: startOfMonth, to: endOfMonth, for: user)
