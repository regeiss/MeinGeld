...
final class DataService: DataServiceProtocol {
    ...
    do {
      let descriptor = FetchDescriptor<Account>(
        predicate: #Predicate<Account> { account in
          account.user?.id == user.id && account.isActive
        },
    ...
    do {
      var descriptor = FetchDescriptor<Transaction>(
        predicate: #Predicate<Transaction> { transaction in
          transaction.account?.user?.id == user.id
        },
      ...
      let descriptor = FetchDescriptor<Budget>(
        predicate: #Predicate<Budget> { budget in
          budget.category == category && budget.month == month
            && budget.year == year && budget.user?.id == user.id
        }
      )
  ...
  func fetchIncomeVsExpenses(for user: User, months: Int) async throws -> [(
    ...
    do {
      ...
      for i in 0..<months {
        ...
        let monthName = calendar.dateFormat("MMM yyyy").string(from: monthDate)
  ...
  private func createSampleTransactions() async throws {
    let accounts = try await fetchAccounts(for: await createSampleUser())
