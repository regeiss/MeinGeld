    ...
    var body: some View {
        List {
            ...
            Section("Verificação de Email") {
                HStack {
                    Image(systemName: emailService.isEmailVerified ? "checkmark.circle.fill" : "exclamationmark.circle.fill")
                        .foregroundColor(emailService.isEmailVerified ? .green : .orange)
                    
                    VStack(alignment: .leading) {
                        Text(emailService.isEmailVerified ? "Email verificado" : "Email não verificado")
                            .fontWeight(.medium)
                    ...
                    if !emailService.isEmailVerified {
                        ...
                        .disabled(emailService.resendCooldown > 0 || emailService.isCheckingVerification)
                    }
                }
            }
